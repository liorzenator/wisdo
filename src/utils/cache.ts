...
export const getUserFeedIds = async (userId: string | Types.ObjectId): Promise<string[] | null> => {
...
export const setUserFeedIds = async (userId: string | Types.ObjectId, ids: (string | Types.ObjectId)[]): Promise<void> => {
  const client = await getRedisClient();
  if (!client) return;
  try {
    const key = keyForUser(userId);
...
export const deleteUserFeed = async (userId: string | Types.ObjectId): Promise<void> => {
  const client = await getRedisClient();
  if (!client) return;
  try {
    const key = keyForUser(userId);
